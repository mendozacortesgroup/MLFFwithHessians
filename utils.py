...
def pad_sequence_2d(sequences, padding_value):
    max_dim = 0
    batch_number = len(sequences)
    # Iterate over the tensors to find the one with the largest dimensions
    for tensor in sequences:
        if tensor.shape[0] >= max_dim:
            max_dim = tensor.shape[0]
    # Initialize the output tensor
    output = torch.zeros([batch_number, max_dim, max_dim])
    # Pad the individual tensors and pack them into the output tensor
    for i, tensor in enumerate(sequences):
        extension = max_dim - tensor.shape[0]
        p2d = (0, extension, 0, extension) # Pads to the right and to the bottom of the tensor
        output[i] = torch.nn.functional.pad(tensor, p2d, "constant", padding_value)
    return output


def stack_with_padding(properties, padding):
    output = defaultdict(list) # Creates a dictionary that doesn't raise "KeyError" and creates an empty list for every new entry
    for p in properties: # Properties is a list of dictionaries
        for k, v in p.items(): # For every key and value in each dictionary in properties:
            output[k].append(torch.as_tensor(v)) # Append the value as a pytorch tensor to the entry k, creating a list of tensors
    for k, v in output.items(): # For every key and value in the "output" dictionary,
        if v[0].dim() == 0: # If the dimension of the first value is 0 (scalar):
            output[k] = torch.stack(v) # Concatenate the list of tensors along a new dimension
        elif k == "hessian": # If the key is "hessian":
            output[k] = pad_sequence_2d(v, padding[k]) # Pad along 2 dimensions and stack the tensors
        else: # For all others:
            output[k] = torch.nn.utils.rnn.pad_sequence(v, True, padding[k]) # Pad the tensors along one dimension and stack them
    return output
